...
from utils import denormalize, to_numpy
...
class GradDisplay:
    ...
    def __init__(self, model, layers, device, mean, std, pred = None, targ = None):
    ...
    def _cam_image(self, norm_image, pred = None, targ = None):
        ...
        for layer, gc in self.gradcam.items():
            ...
            cam_heatmap, cam_result = visualize_cam(
                mask,
                denormalize(norm_image, self.mean, self.std, out_type='tensor').clone().unsqueeze_(0).to(self.device)
            )
            heatmap[layer], result[layer] = to_numpy(cam_heatmap), to_numpy(cam_result)
        if self.print_labels:
          return {
              'image': denormalize(norm_image, self.mean, self.std),
              'heatmap': heatmap,
              'result': result,
              'pred': pred,
              'targ': targ
    ...
    def _plot_view(self, view, fig, row_num, ncols, metric):
        ...
        if self.print_labels:
          sub.set_title("Predicted: %s\nActual: %s" % (view['pred'], view['targ']))
        else:
    ...
    def cam(self, norm_image_list):
        ...
        """
        if self.print_labels:
          for norm_image, p, t in zip(norm_image_list, self.pred, self.targ):
              self.views.append(self._cam_image(norm_image, p, t))
        else:
    ...
    def __call__(self, norm_image_list, plot_path, print_labels = False):
